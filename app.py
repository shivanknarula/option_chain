...
def print_oi(num, step, nearest, url):
    ...
    result = []
    result.append(strPurple( "Expiry".ljust(12, " ") + " =>  ") + strLightPurple(currExpiryDate))
    result.append(print_hr())
    for item in data['records']['data']:
        if item["expiryDate"] == currExpiryDate:
            if item["strikePrice"] == strike and item["strikePrice"] < start_strike + (step * num * 2):
                result.append(strCyan(str(item["strikePrice"])) + strGreen(" CE ") + "[ " + strBold(
                    str(item["CE"]["openInterest"]).rjust(10, " ")) + " ]" + strRed(" PE ") + "[ " + strBold(
...
def option_chain():
    set_header()
    option_chain_data = {
        "nifty_header": print_header("Nifty", nf_ul, nf_nearest),
        "nifty_oi": print_oi(5, 50, nf_nearest, url_nf),
        "banknifty_header": print_header("Bank Nifty", bnf_ul, bnf_nearest),
        "banknifty_oi": print_oi(10, 100, bnf_nearest, url_bnf),
    }
    return render_template("option_chain.html", data=option_chain_data)
